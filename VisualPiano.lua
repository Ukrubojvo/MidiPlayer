				...
				if k then
					...
					if noteIndex then
						...
						if no_note_off_enabled then
							local delta = random_note_enabled and (_math.random(1, 15) * 0.01) or 0.05
...
run(function()
	...
	SettingsTab:Toggle({
		...
		Callback = function(v)
			...
			WindUI:Notify({
				Title = "MidiPlayer",
				Content = (auto_velocity_enabled and "🔧 AutoVelocity ON" or "🔧 AutoVelocity OFF"),
				Duration = 3,
				Icon = "bird",
			})
		end
	...
	SettingsTab:Toggle({
		...
		Callback = function(v)
			...
			WindUI:Notify({
				Title = "MidiPlayer",
				Content = (loop_enabled and "🔁 Loop Mode ON" or "🔁 Loop Mode OFF"),
				Duration = 3,
				Icon = "bird",
			})
		end
